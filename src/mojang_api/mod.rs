...
impl APIClient {
    pub async fn get_user_by_username<S: AsRef<str>>(
        &self,
        username: S,
    ) -> Result<UserByUsernameResponse, Error> {
        self.process_json(self.http_client.get(format!(
            "https://api.mojang.com/users/profiles/minecraft/{}",
            username.as_ref()
        )))
        .await
    }
    pub async fn get_username_change_history<S: AsRef<str>>(
        &self,
        uuid: S,
    ) -> Result<UserByUsernameResponse, Error> {
        self.process_json(self.http_client.get(format!(
